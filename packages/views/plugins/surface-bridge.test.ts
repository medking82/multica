...
describe("surface bridge", () => {
  ...
  it("forwards the Skills summary endpoint through the plugin Action API", async () => {
    const { port } = connectedBridge();
    port.postMessage({ id: "skills", kind: "action", method: "GET", path: "/skills" });
    await vi.waitFor(() => expect(mockCall).toHaveBeenCalledWith(
      "installation-1",
      expect.objectContaining({ method: "GET", path: "/skills" }),
    ));
  });
