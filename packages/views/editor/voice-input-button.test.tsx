...
vi.mock("@multica/core/api", async (importOriginal) => {
  ...
  return {
    ...actual,
    api: { transcribeAudio: mocks.transcribeAudio },
  };
});
...
function editorRef(insert = vi.fn(() => true)) {
  return {
    ref: {
      current: {
        insertPlainTextAtSelection: insert,
        focusForNativeInput: vi.fn(() => true),
...
describe("VoiceInputButton", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    configStore.getState().setAudioTranscriptionEnabled(true);
  ...
  it("stays absent until the server advertises transcription", () => {
    configStore.getState().setAudioTranscriptionEnabled(false);
    const { ref } = editorRef();

    renderWithI18n(<VoiceInputButton editorRef={ref} />);

    expect(screen.queryByRole("button", { name: "Dictate" })).not.toBeInTheDocument();
  });

  it("records, transcribes, and inserts at the live selection without submitting", async () => {
    mocks.transcribeAudio.mockResolvedValue("Ship the reviewed change.");
    const { ref, insert } = editorRef();
    const onBeforeRecord = vi.fn();

    renderWithI18n(
      <VoiceInputButton editorRef={ref} onBeforeRecord={onBeforeRecord} />,
    );

    fireEvent.click(screen.getByRole("button", { name: "Dictate" }));
    expect(onBeforeRecord).toHaveBeenCalledOnce();
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "Recording — click to stop" }),
      ).toBeInTheDocument(),
    );
    ...
    const [file, signal] = mocks.transcribeAudio.mock.calls[0] as [File, AbortSignal];
    expect(file.name).toBe("recording.webm");
    expect(file.type).toBe("audio/webm;codecs=opus");
    expect(signal).toBeInstanceOf(AbortSignal);
    await waitFor(() =>
      expect(insert).toHaveBeenCalledWith("Ship the reviewed change."),
    );
    expect(stopTrack).toHaveBeenCalledOnce();
    expect(screen.getByRole("button", { name: "Dictate" })).toBeInTheDocument();
  });

  it("keeps the editor unchanged when microphone permission is denied", async () => {
    ...
    await waitFor(() =>
      expect(mocks.toastError).toHaveBeenCalledWith(
        "Microphone access was denied. Allow it in your system or browser settings.",
      ),
    );
  ...
  it("automatically stops a recording at the two-minute privacy bound", async () => {
    vi.useFakeTimers();
    mocks.transcribeAudio.mockResolvedValue("Bounded recording.");
    const { ref, insert } = editorRef();
    ...
    expect(mocks.toastInfo).toHaveBeenCalledWith(
      "Recording stopped after 2 minutes.",
    );
    expect(mocks.transcribeAudio).toHaveBeenCalledOnce();
    expect(insert).toHaveBeenCalledWith("Bounded recording.");
  });
  ...
  it.each([false, true])("native dictation is authoritative with API capability %s", async (enabled) => {
    configStore.getState().setAudioTranscriptionEnabled(enabled);
    const { ref, insert } = editorRef();
