...
describe("ContentEditor", () => {
  ...
  it("inserts a transcription at the live selection as plain text", () => {
    const ref = createRef<ContentEditorRef>();
    render(<ContentEditor ref={ref} />);

    expect(ref.current?.insertPlainTextAtSelection("Review <main> & ship")).toBe(true);
    expect(mockFocus).toHaveBeenCalledTimes(1);
    expect(mockInsertContent).toHaveBeenCalledWith({
      type: "text",
      text: "Review <main> & ship",
    });
  });
