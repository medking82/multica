...
function editorWithSlashText(text = "/skills") {
...
describe("plugin composer insertion", () => {
  ...
  it("inserts Markdown only into the same unchanged editor and range", () => {
    ...
    expect(insertPluginComposerMarkdown(invocation, editor, "[/review](slash://skill/skill-1)")).toBe(true);
    expect(insertContentAt).toHaveBeenCalledWith(
      { from: 0, to: 7 },
      "[/review](slash://skill/skill-1)",
