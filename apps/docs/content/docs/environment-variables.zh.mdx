...
| `MULTICA_LLM_TRANSCRIPTION_MODEL` | 空 | 使用指定 audio model 显式启用 microphone transcription，例如 `gpt-4o-mini-transcribe` |
...
有三个功能会使用这一层，它们都会把私密内容发送到你配置的 endpoint：

- **对话标题自动生成** —— 发送新对话中用户的第一条消息，原文发送。附件不会包含在内。
- **后续提问建议**（智能体回复下方的按钮）—— 发送对话末尾的内容：最多 6 条消息，其中被追问的那条回复上限 3000 字符，更早的每条上限 800 字符。
- **Voice input** —— 只有用户主动开始并停止 microphone 后，才会向上游 transcription endpoint 发送一段录音。客户端在 2 分钟后自动停止，服务端拒绝超过 10 MiB 的录音；Multica 只在 memory 中持有 audio bytes，不会写入 database、attachment storage 或 disk。上游 provider 自己的 retention policy 仍然适用。

`MULTICA_LLM_TRANSCRIPTION_MODEL` 是独立的显式 opt-in。即使标题或后续提问已启用，也可以把它留空以隐藏 microphone。只应设置为当前 endpoint 支持的 audio transcription model；voice input 会复用同一组 API key、base URL 和 retry policy。

API key 与 base URL 都为空时，这一层关闭，不会发出任何上游请求——上面三个功能都不再发送内容。如果你的政策不允许这一层把私密内容发到部署之外，这就是受支持的配置方式：对话继续使用客户端根据第一条消息生成的标题，后续提问按钮和 microphone 不再出现，其余功能不受影响。
