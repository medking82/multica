...
| `MULTICA_LLM_TRANSCRIPTION_MODEL` | empty | Explicitly enables microphone transcription with this audio model; for example, `gpt-4o-mini-transcribe` |
...
Three features use this layer, and each one sends private content to the endpoint you configure:
...
- **Voice input** — only after the user starts and stops the microphone, one audio recording is sent to the upstream transcription endpoint. The client stops recording after 2 minutes, the server rejects recordings over 10 MiB, and Multica keeps the bytes in memory only: it does not write them to its database, attachment storage, or disk. The upstream provider's retention policy still applies.

`MULTICA_LLM_TRANSCRIPTION_MODEL` is a separate, explicit opt-in. Leave it empty to keep the microphone hidden even when chat titles or follow-up questions use this layer. Set it only to an audio transcription model supported by your configured endpoint; voice input reuses the same API key, base URL, and retry policy.

When both the API key and the base URL are empty, this layer is disabled and makes no upstream request at all — none of the three features above sends anything. That is the supported configuration when your policy does not allow this layer to send private content off the deployment: chat sessions keep the title the client derives from the first message, the follow-up question buttons and microphone do not appear, and everything else is unaffected.
