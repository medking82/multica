...
| `MULTICA_LLM_TRANSCRIPTION_MODEL` | 空 | 指定した音声モデルでマイク文字起こしを明示的に有効化。例: `gpt-4o-mini-transcribe` |
...
このレイヤーを使う機能は 3 つあり、すべて設定した endpoint に非公開の内容を送信します。
...
- **音声入力** — ユーザーがマイク録音を開始して停止した場合に限り、1 件の録音を upstream transcription endpoint に送ります。クライアントは 2 分後に録音を停止し、サーバーは 10 MiB を超える録音を拒否します。Multica は audio bytes を memory 内だけに保持し、database、attachment storage、disk には書き込みません。Upstream provider の retention policy は引き続き適用されます。

`MULTICA_LLM_TRANSCRIPTION_MODEL` は独立した明示的 opt-in です。チャットタイトルやフォローアップ質問でこのレイヤーを使っていても、空のままなら microphone は表示されません。設定した endpoint が対応する audio transcription model だけを指定してください。Voice input は同じ API key、base URL、retry policy を再利用します。

API key と base URL の両方が空の場合、このレイヤーは無効になり、upstream へのリクエストを一切行いません。上記 3 つの機能はすべて何も送信しなくなります。このレイヤーから非公開の内容を deployment 外へ出せないポリシーでは、これがサポートされた構成です。セッションはクライアントが最初のメッセージから生成したタイトルをそのまま使い、フォローアップ質問のボタンと microphone は表示されず、他の機能への影響はありません。
