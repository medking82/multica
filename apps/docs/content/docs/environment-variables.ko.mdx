...
| `MULTICA_LLM_TRANSCRIPTION_MODEL` | 비어 있음 | 지정한 오디오 모델로 마이크 음성 변환을 명시적으로 활성화합니다. 예: `gpt-4o-mini-transcribe` |
...
이 레이어를 사용하는 기능은 세 가지이며, 모두 설정한 endpoint로 비공개 내용을 전송합니다.
...
- **음성 입력** — 사용자가 마이크 녹음을 시작하고 중지한 경우에만 한 개의 녹음이 upstream transcription endpoint로 전송됩니다. 클라이언트는 2분 후 녹음을 중지하고 서버는 10 MiB를 초과하는 녹음을 거부합니다. Multica는 audio bytes를 memory에만 보관하며 database, attachment storage 또는 disk에 기록하지 않습니다. Upstream provider의 retention policy는 그대로 적용됩니다.

`MULTICA_LLM_TRANSCRIPTION_MODEL`은 별도의 명시적 opt-in입니다. 채팅 제목이나 후속 질문에 이 레이어를 사용하더라도 비워 두면 microphone이 숨겨집니다. 설정한 endpoint가 지원하는 audio transcription model만 지정하세요. Voice input은 같은 API key, base URL 및 retry policy를 재사용합니다.

API key와 base URL이 모두 비어 있으면 이 레이어가 꺼지고 upstream 요청을 전혀 보내지 않습니다. 위 세 기능 모두 아무것도 전송하지 않습니다. 이 레이어가 비공개 내용을 deployment 밖으로 보내면 안 되는 정책이라면 이것이 지원되는 구성입니다. 세션은 클라이언트가 첫 메시지에서 만든 제목을 그대로 사용하고, 후속 질문 버튼과 microphone은 표시되지 않으며, 나머지 기능은 영향을 받지 않습니다.
