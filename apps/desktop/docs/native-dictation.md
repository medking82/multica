...
This Windows custom build reserves `Ctrl+Alt+Shift+D` in Multica even when
Codex dictation is not configured or running. On layouts where
`AltGr+Shift+D` produces a character (for example `Đ`), that chord cannot type
the character in Multica; use another text-entry method. This is an explicit
trade-off of the fixed shortcut, not layout-independent keyboard behavior.
A failed dispatch needs a fresh mic click after correcting the problem; an
earlier click is never retained or automatically retried.
