...
import {
  ...
  configureCustomWindowsUpdateChannel,
  setupAutoUpdater,
} from "./updater";
...
describe("custom Windows update feed", () => {
  it("pins the fork's latest feed and disables prereleases and downgrades", () => {
    const updater = {
    ...
    configureCustomWindowsUpdateChannel(updater, "win32", "x64");
    expect(updater.setFeedURL).toHaveBeenCalledExactlyOnceWith({
      provider: "github", owner: "medking82", repo: "multica", channel: "latest",
    });
  ...
  it.each([["darwin", "x64"], ["win32", "arm64"], ["linux", "x64"]] as const)(
    "does not migrate unsupported %s/%s builds", (platform, arch) => {
      const updater = {
        channel: null, allowPrerelease: true, allowDowngrade: true, setFeedURL: vi.fn(),
      };
      configureCustomWindowsUpdateChannel(updater, platform, arch);
...
describe("setupAutoUpdater", () => {
  ...
  it.each([true, false])("guards install-on-quit without stopping an agent (safe=%s)", async (safe) => {
    ...
    emitUpdater("update-downloaded", { version: "0.4.36-custom.2" });
  ...
  it("rehydrates pending and deferred state instead of losing the download event", async () => {
    ...
    await invokeIpc("updater:get-preferences");
    emitUpdater("update-downloaded", { version: "0.4.36-custom.2" });
    expect(await invokeIpc("updater:get-install-state")).toEqual({ status: "ready", version: "0.4.36-custom.2" });
    await invokeIpc("updater:install");
    expect(await invokeIpc("updater:get-install-state")).toMatchObject({ status: "deferred", version: "0.4.36-custom.2" });
  });
  ...
  it("keeps checking and retains the latest download version during a probe", async () => {
    ...
    emitUpdater("update-downloaded", { version: "0.4.36-custom.2" });
    const installing = invokeIpc("updater:install");
    emitUpdater("update-downloaded", { version: "0.4.36-custom.3" });
    expect(await invokeIpc("updater:get-install-state")).toEqual({ status: "checking", version: "0.4.36-custom.3" });
    resolve(blocked);
    await installing;
    expect(await invokeIpc("updater:get-install-state")).toMatchObject({ status: "deferred", version: "0.4.36-custom.3" });
  });
  ...
  it("coalesces duplicate install IPC and returns a visible checking/deferred state", async () => {
    ...
    expect(await invokeIpc("updater:get-install-state")).toEqual({ status: "checking", version: "0.4.36-custom.2" });
    resolve(blocked);
