...
func TestEnqueueDeferredChannelIssueTaskRefreshesRuntimeSchedule(t *testing.T) {
	...
	if task.Status != "deferred" || len(wakeup.calls) != 1 {
		t.Fatalf("channel task/status wakeups = %q/%+v, want deferred with one schedule refresh", task.Status, wakeup.calls)
	}
	if wakeup.calls[0].runtimeID != util.UUIDToString(task.RuntimeID) || wakeup.calls[0].taskID != "" {
		t.Fatalf("channel schedule wakeup = %+v, want runtime %s without task id", wakeup.calls[0], util.UUIDToString(task.RuntimeID))
	}
}
