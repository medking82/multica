...
## Fork Custom Desktop Release

- Fork-only Windows custom Desktop releases use `codex/desktop-custom`, the repository-local `.sop/sop.py`, and `docs/custom-desktop.md`. They publish only `desktop-v*-custom.*` to `medking82/multica`; this exception does not authorize upstream, CLI, Homebrew, or server production releases.
